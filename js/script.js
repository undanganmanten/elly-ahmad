...
var countDownDate = new Date("Oct 23, 2021 11:00:00").getTime();
...
var firebaseConfig = {
  ...
  function add_task(){
    ...
    if(input_box.value.length != 0 && input_date.value.length != 0){
      ...
      var key = firebase.database().ref().child("elly").push().key;
      var task = {
...
function create_unfinished_task(){
    ...
    task_array = [];
    firebase.database().ref("elly").once('value', function(snapshot) {
